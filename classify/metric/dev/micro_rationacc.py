...
def compute_raionale_metrics(preds: List[Tuple[torch.FloatTensor, torch.FloatTensor]],
                targets: List[Dict[str, torch.LongTensor]],
                threshold: float=0.1,
                absolute_threshold: bool=False) -> torch.float:
        """
        ...
        with torch.no_grad():
            ...
            for (cost, alignment), target in zip(preds, targets):
                if len(alignment.shape) == 3:
                    ...
                    if absolute_threshold:
                        ...
                        column_alignment = (column_alignment>= threshold/prod(column_alignment.shape[-2:])).float()  #n
                        row_alignment = (row_alignment>= threshold/prod(column_alignment.shape[-2:])).float() #m
                    ...
                    if absolute_threshold:
                        ...
                        alignment = (alignment >= threshold/prod(alignment.shape[-2:])).float()  #n
                ...
                if 'lengths' in target:
                    ...
                    predict_column_r =  rationale_sent_to_token(target['lengths'], predict_column_r)
                ...
                correct_true_c += np.sum((column_r == 1) * (predict_column_r == 1))#.float()
            ...
            p_all = (correct_true + correct_true_c)/(predicted_true + predicted_true_c + epsilon)
            r_all = (correct_true + correct_true_c)/(target_true + target_true_c + epsilon)
            ...
            precision_c = precision_score(all_column_r, all_predict_column_r, average='macro')
