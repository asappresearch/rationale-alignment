    ...
    def forward(
        ...
        if threshold:
            if self.args.absolute_threshold:
                row_alignment = (
                    row_alignment * (row_alignment >= threshold).float().cuda()
                )
                column_alignment = (
                    column_alignment * (column_alignment >= threshold).float().cuda()
                )
            else:
                mask = (
                    (
                        torch.rand(row_alignment.size(0), row_alignment.size(1))
                        >= threshold
                    )
                    .float()
                    .cuda()
