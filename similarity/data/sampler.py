...
class Sampler:
    ...
    def sample_negatives(self, id: str, available_ids: Set[str]) -> Set[str]:
        ...
        if len(negative_ids) < self.num_negatives:
            candidate_ids = [
                candidate_id
                for candidate_id in self.data.id_list
                if candidate_id in valid_negative_ids
            ]
    ...
    def sample(
        ...
        for i in trange(0, len(self.data), self.batch_size):
            ...
            batch_available_ids: Set[str] = set.union(
                ...
                *[
                    self.data.id_mapping[document_id]["similar"]
            ...
            for document_id in batch_document_ids:
                ...
                if self.resample_negatives or len(dissimilar_ids) == 0:
                    dissimilar_ids = self.sample_negatives(
                        id=document_id, available_ids=batch_available_ids
                    )
                    self.data.id_mapping[document_id]["dissimilar"] = dissimilar_ids

                # Subsample positives if too many
                if (
                    self.num_positives is not None
                    and len(similar_ids) > self.num_positives
                ):
                ...
                if (
                    ...
                    and len(dissimilar_ids) > self.num_negatives
                ):
                ...
                for similar_id in similar_ids:
                    batch_scope.append(
                        (
                            torch.LongTensor(id_to_scope[document_id]).to(self.device),
                ...
                for dissimilar_id in dissimilar_ids:
                    batch_scope.append(
                        (
                            torch.LongTensor(id_to_scope[document_id]).to(self.device),
                            torch.LongTensor(id_to_scope[dissimilar_id]).to(
                                self.device
                            ),
                        )
                    )
