...
class AlignmentSum(AlignmentAverageMetric):
    def _compute_one(
        self, cost: torch.FloatTensor, alignment: torch.FloatTensor, target: int
    ) -> torch.float:
...
class AlignmentCount(AlignmentAverageMetric):
    ...
    def _compute_one(
        ...
        count = torch.sum(alignment > 1e-10).float()
    ...
    def __str__(self) -> str:
        ...
        if self.normalize != "none":
            string += "_normalized_by_" + (
                "min_nm" if self.normalize == "min" else "nm"
            )
...
class AlignmentMarginalError(AlignmentAverageMetric):
    ...
    def _compute_one(
        ...
        marginal = torch.ones(
            alignment.size(marginal_dim), device=device
        ) / alignment.size(marginal_dim)
